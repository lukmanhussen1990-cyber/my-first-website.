...
def write_output() -> tuple[int, int]:
    ...
    # Flatten sections into parts of at most MAX_CMDS_PER_PART commands.
    parts: list[list[str]] = []
    cur: list[str] = []
    cmds = 0
    total = 0
    for name, lines in g.sections:
        for line in lines:
            ...
            if is_cmd and cmds >= MAX_CMDS_PER_PART:
                parts.append(cur)
                cur, cmds = [f"# (continued: {name})"], 0
            cur.append(line)
            if is_cmd:
                cmds += 1
                total += 1
    ...
    for a, b, c, d, e, f in split_box(*BOUNDS_FROM, *BOUNDS_TO):
        clear.append(f"fill {rc(a)} {rc(b)} {rc(c)} {rc(d)} {rc(e)} {rc(f)} {AIR}")
    (OUT / "clear.mcfunction").write_text("\n".join(clear) + "\n")
